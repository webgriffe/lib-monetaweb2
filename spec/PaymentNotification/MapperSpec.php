...
class MapperSpec extends ObjectBehavior
{
    ...
    public function it_should_create_error_payment_result_info_with_error_response()
    {
        $request = new ServerRequest('POST', 'any uri');
        $parsedBody = [
            'paymentid' => '123456789012345678',
        ...
        $expectedPaymentResult = new PaymentResultErrorInfo('code', 'message', '123456789012345678');
        $this->map($request)->shouldBeLike($expectedPaymentResult);
    }
    ...
    public function it_should_throw_exception_when_required_fields_are_missing()
    {
        ...
        $this->shouldThrow(\InvalidArgumentException::class)->duringMap(
            $request->withParsedBody(['paymentid' => '1'])
        );
        $this->shouldThrow(\InvalidArgumentException::class)->duringMap(
        ...
        $this->shouldThrow(\InvalidArgumentException::class)->duringMap(
            $request->withParsedBody(['mybankid' => '1'])
        );
        $this->shouldThrow(\InvalidArgumentException::class)->duringMap(
            $request->withParsedBody(['mybankid' => '1', 'paymentid' => '1'])
        );
        $this->shouldThrow(\InvalidArgumentException::class)->duringMap(
            $request->withParsedBody(['mybankid' => '1', 'result' => '000'])
        );
    }
}
